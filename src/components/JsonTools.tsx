...
export const JsonTools: React.FC<JsonToolsProps> = ({ darkMode = false }) => {
  ...
  const [diffResult, setDiffResult] = useState<JsonDiffResult[]>([]);
  ...
  const handleJsonChange = (value: string) => {
    ...
    if (!result.isValid && result.error) {
      message.error(`JSON 格式错误: ${result.error}`);
    }
  };
  ...
  const compareJsons = () => {
    ...
    try {
      ...
      const diffs = generateJsonDiff(leftParsed, rightParsed);
      setDiffResult(diffs);
      message.success(`发现 ${diffs.filter(d => d.type !== 'unchanged').length} 处差异`);
    } catch (error) {
      message.error('JSON 格式错误，无法比较');
    }
  };

  // 生成 JSON 差异
  const generateJsonDiff = (left: any, right: any, path: string = ''): JsonDiffResult[] => {
    ...
    for (const key of allKeys) {
      ...
      if (leftHasKey && rightHasKey) {
        results.push(...generateJsonDiff(left[key], right[key], currentPath));
